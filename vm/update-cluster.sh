  ...
  if [ "$NO_REBUILD" = true ]; then
    ...
    ssh $SSH_OPTS "$SSH_USER@$node_ip" \
      "sudo tar xzf /tmp/coredocker-update.tar.gz -C $REPO_DIR && \
       ...
       sudo rm -rf data/backup/__system__/etcd/cluster-config.json && sudo docker rm -f core-docker-backend core-docker-frontend core-docker-proxy core-docker-coredns core-docker-keepalived-dns 2>/dev/null; \
